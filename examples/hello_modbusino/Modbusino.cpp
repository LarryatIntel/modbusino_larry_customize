...
// suppose sent tab_reg = [0c 10 00 04 00 02 04 10 c8 10 c8 40 98]
// then the reply should be [0c 10 00 04 00 02 01 14]
static void reply(uint16_t *tab_reg, uint16_t nb_reg, uint8_t *req,
                  uint8_t req_length, uint8_t _slave)
{
    uint8_t slave = req[_MODBUS_RTU_SLAVE]; // 0c
    uint8_t function = req[_MODBUS_RTU_FUNCTION];  // 10
    uint16_t address =
        (req[_MODBUS_RTU_FUNCTION + 1] << 8) + req[_MODBUS_RTU_FUNCTION + 2];   // address = 0004
    uint16_t nb =
        (req[_MODBUS_RTU_FUNCTION + 3] << 8) + req[_MODBUS_RTU_FUNCTION + 4];   // nb = 0002
    ...
    // according to example, address is 0004, nb = 0002
    if (address + nb > nb_reg) {
        rsp_length = response_exception(
            slave, function, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp);
    } else {
        req_length -= _MODBUS_RTU_CHECKSUM_LENGTH;    // now req_length = 13-2=11
